<?php
  if(isset($_GET["no"])){
    ...
    header("Location:?m=settings/themes&f=editcssjs&mode=".$_GET["mode"]."&nh=1&ID=".$last_id);
    die();
  }
?>
<form action="?m=settings/themes&f=newcssjs&mode=<?php echo $_GET["mode"]?>&no=1" method="post">
  <?php
    if($_GET["mode"]=="css"){
